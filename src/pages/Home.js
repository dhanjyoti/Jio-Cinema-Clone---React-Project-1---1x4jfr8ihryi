...
const ShowSection = ({ category, onFetch }) => {
  ...
  return (
    <div key={category} className="select-none flex flex-col">
      ...
      <div className="relative group">
        ...
        <ScrollContainer
          onScroll={onScrollerScrolls}
          className={c}
          ref={scroller}
        >
          <div
            className={"group relative flex flex-row items-center gap-3 px-5"}
          >
            {data.map((show) => {
              return (
                <Card
        ...
        <div
          ...
          className="cursor-pointer top-0 transition-all duration-500 opacity-0 invisible group-hover:visible group-hover:opacity-100 absolute h-full right-0 text-white w-[64px] flex items-center justify-center"
          style={{
            background:
              "linear-gradient(270deg, rgb(0, 0, 0) 0%, rgba(0, 0, 0, 0) 96.38%)",
          }}
        >
          <svg
            height={20}
        ...
        {scrolledLeft && (
          <div
            ...
            className="cursor-pointer top-0 transition-all duration-500 opacity-0 invisible group-hover:visible group-hover:opacity-100 absolute h-full left-0 text-white w-[64px] flex items-center justify-center"
            style={{
              background:
                "linear-gradient(90deg, rgb(0, 0, 0) 0%, rgba(0, 0, 0, 0) 96.38%)",
            }}
          >
            <svg
...
const Home = () => {
  ...
  return (
    ...
    <>
      {Images.length > 0 && (
        ...
        >
          {Images.map((cat, index) => (
            <SplideSlide key={cat}>
              <img
                className="w-full aspect-video object-cover object-top max-h-[60vh]"
                src={cat}
                alt={cat}
              />
              <div
                className="absolute bottom-0 left-0 right-0 w-full pb-5"
                style={{
                  background:
      ...
      {searchParams.get("type")?.toLowerCase() !== "news" && (
            ...
            <ShowSection
              ...
              onFetch={(data) => {
                setShowCollection((prev) => [
                  ...prev,
                  { category: s, items: data.splice(0, 2) },
                ]);
              }}
            />
          ))}
